...
class CameraPlacementProblem:
    ...
    def __post_init__(self):
        ...
        # Track best solution for GIF generation
        self.best_x = None
...
def create_optimization_gif(
    ...
    for i, idx in enumerate(indices):
        ...
        eval_num = (idx + 1) * 10  # Approx eval number
...
def save_final_result(
    ...
    for i, cam in enumerate(cameras):
        info_text += f"Cam {i+1}: ({cam.x:.0f},{cam.y:.0f}) FOV={np.rad2deg(cam.hfov):.0f}° range={cam.max_range:.0f}\n"
...
def run_comparison(
    ...
    print("=" * 70)
    print(f"OPTIMIZER COMPARISON: {scenario.upper()} scenario")
    ...
    if skip_slow:
        # Skip slower methods for quick testing
    ...
    for name in methods_to_run:
        ...
        t0 = time.time()
        try:
            # Run with method-specific params
            if name == "PGO":
                res = runner(
                    problem.objective,
                    problem.bounds,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Compare optimization methods for camera placement')
